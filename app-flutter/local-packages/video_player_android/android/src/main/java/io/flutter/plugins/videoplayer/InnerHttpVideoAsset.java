...
final class HttpVideoAsset extends VideoAsset {
  ...
  MediaSource.Factory getMediaSourceFactory(Context context) {
    try {
      if (cronetEngine == null) {
        cronetEngine = new CronetEngine.Builder(context.getApplicationContext())
            .enableHttp2(true)
            .enableQuic(true)
            .build();
      }
      CronetDataSource.Factory cronetDataSourceFactory = new CronetDataSource.Factory(cronetEngine, executor);
      return new DefaultMediaSourceFactory(context).setDataSourceFactory(
          new DefaultDataSource.Factory(context, cronetDataSourceFactory));
    } catch (Exception e) {}
   ...
   * @return configured factory, or {@code null} if not needed for this asset
   *         type.
   */
  @VisibleForTesting
  ...
  // TODO: Migrate to stable API, see
  // https://github.com/flutter/flutter/issues/147039.
